...
// Kernels
__global__ void global_AvProduct(float* g_MatA, float* g_VecV, float* g_VecW, int N);
...
/* Global Memory Implementation of Av_Product() kernel */
__global__ void global_AvProduct(float* g_MatA, float* g_VecV, float* g_VecW, int N)
{
    int matIndex = 0;
...
int main(int argc, char** argv)
{
    ...
    for (int i=0; i < max_iteration; i++)
    {
        ...
        global_AvProduct<<<blocksPerGrid, threadsPerBlock, sharedMemSize>>>(d_MatA, d_VecV, d_VecW, N);
        cudaDeviceSynchronize();
